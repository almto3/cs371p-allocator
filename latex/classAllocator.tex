...
O(1) in space O(1) in time after deallocation adjacent free blocks must be coalesced throw an invalid\-\_\-argument exception, if p is invalid deallocate takes an occupied block and frees it, will throw an exception in case the given pointer does not point to the first sentinel of an occupied block \hypertarget{classAllocator_af156a6a50a8c62c70e40cf342a3b64cb}{\index{Allocator@{Allocator}!destroy@{destroy}}
...
O(1) in space O(1) in time interprets bytes to ints from the heap 
